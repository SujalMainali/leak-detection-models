...
from data_preprocessing.cnn_preprocessing import preprocess_cnn_long_csv
...
def _require_torch():
    try:
        ...
        raise RuntimeError(
            "PyTorch is required to train the CNN model. Install it in LeakEnv: `pip install torch`."
        ) from e
...
def _load_or_build_processed(train_ids: np.ndarray | None) -> dict:
    if config.CNN_READY_NPZ_PATH.exists() and config.CNN_SCALERS_PATH.exists() and not config.REBUILD_PROCESSED_ON_RUN:
        data = np.load(config.CNN_READY_NPZ_PATH, allow_pickle=True)
        scalers = joblib.load(config.CNN_SCALERS_PATH)
        return {
            "X": data["X"],
            "y": data["y"],
    ...
    res, artifacts, meta = preprocess_cnn_long_csv(
        ...
        out_npz_path=config.CNN_READY_NPZ_PATH,
        out_scalers_path=config.CNN_SCALERS_PATH,
        out_meta_path=config.CNN_META_PATH,
    )
...
def _make_splits(scenario_ids: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if config.REBUILD_SHARED_SPLIT_IDS_ON_RUN and config.SPLIT_IDS_DIR.exists():
        for p in config.SPLIT_IDS_DIR.glob("cnn_ids_*.csv"):
            p.unlink()

    if (config.SPLIT_IDS_DIR / "cnn_ids_train.csv").exists():
        ...
        save_split_ids(ids_split, splits_dir=config.SPLIT_IDS_DIR, id_col=config.SCENARIO_ID_COL, prefix="cnn_ids")
...
def main() -> None:
    ...
    # We only need scenario_ids for splitting; read minimal info from CSV.
    ...
    X = processed["X"].astype(np.float32)
    ...
    if not np.isfinite(X).all():
        raise ValueError(
            ...
            "Rebuild the processed files by deleting data/processed/cnn_ready.npz (and scalers) "
            "or setting REBUILD_PROCESSED_ON_RUN=True in models/cnn/config.py."
        )
