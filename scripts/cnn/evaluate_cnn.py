...
from models.cnn.utils import evaluate_targets, save_metrics, save_metrics_json, save_predictions
...
def _require_torch():
    try:
        ...
        raise RuntimeError(
            "PyTorch is required to evaluate the CNN model. Install it in LeakEnv: `pip install torch`."
        ) from e
...
def main() -> None:
    ...
    # Inverse-transform for human metrics
    ...
    try:
        ...
        for i, t in enumerate(target_columns):
            ...
            plt.xlabel(f"True {t}")
            plt.ylabel(f"Pred {t}")
            plt.title(f"CNN: Pred vs True ({t})")
            out_path = config.PLOTS_DIR / f"pred_vs_true__{t}.png"
            plt.tight_layout()
            plt.savefig(out_path, dpi=150)
            plt.close()
    except Exception:
        # If matplotlib isn't installed, metrics/predictions are still saved.
        pass
