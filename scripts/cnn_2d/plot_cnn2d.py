...
def _plot_loss_curves(*, plt, history_csv: Path, out_dir: Path) -> None:
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("CNN (2D): Training vs Validation Loss")
    plt.legend()
    plt.tight_layout()
...
def _plot_parity_and_residuals(*, plt, y_true: pd.DataFrame, y_pred: pd.DataFrame, out_base: Path) -> None:
    ...
    for t in target_columns:
        ...
        plt.title(f"CNN (2D): Residuals ({t})")
        plt.tight_layout()
        ...
        plt.ylabel("Count")
        plt.title(f"CNN (2D): Residual Distribution ({t})")
        plt.tight_layout()
...
def main() -> None:
    ...
    if not pred_path.exists():
        raise FileNotFoundError(f"Missing predictions file: {pred_path}. Run scripts/cnn_2d/predict_cnn2d.py first.")
